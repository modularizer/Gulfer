...
export async function generateTypes(options: GenerateTypesOptions): Promise<GenerateTypesResult> {
  ...
  if (!resolvedTsconfigPath) {
    ...
    if (!resolvedTsconfigPath) {
      throw new Error('Could not find tsconfig.json. Please provide tsconfigPath option.');
    }
  }
  
  // Create temporary directory for type extraction
  const tempDir = path.join(path.dirname(outputPath), '.temp-types');
  ...
  try {
    ...
    const tempFilePath = createTempTypeFile(sourceFilePath, exportName, tempDir, tableName);
...
`;
    ...
    if (tableName) {
      ...
      const relativeSourcePath = path.relative(outputDir, sourceFilePath).replace(/\\/g, '/').replace(/\.ts$/, '');
