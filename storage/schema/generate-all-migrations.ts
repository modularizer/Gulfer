...
function generateMigrationsIndex(module: ModuleConfig): void {
  if (!fs.existsSync(module.migrationsDir)) {
    console.warn(`⚠️  Migrations directory not found: ${module.migrationsDir}`);
  ...
  if (files.length === 0) {
    console.warn(`⚠️  No migration files found for ${module.name}.`);
    fs.writeFileSync(module.indexFile, `export default [] as Array<{ name: string; hash: string; sql: string }>;\n`);
    return;
  }
...
function generateModuleMigrations(module: ModuleConfig): void {
  console.log(`🔧 Generating ${module.name} migrations...`);
  ...
  try {
    ...
    if (sqlFiles.length === 0) {
      console.error('❌ No migration files were generated.');
      if (output) {
    ...
    console.log(`✅ ${module.name} migrations generated successfully!`);
  } catch (error: any) {
    console.error(`❌ Error generating ${module.name} migrations:`, error);
...
function generateAll(): void {
  console.log('🚀 Generating all migrations...\n');
