...
export abstract class ScoringMethod {
  ...
  /**
   * Validate that a score value is valid for this scoring method
   * If valueSchema is provided, it will be checked first, then this function
   */
...
/**
 * Validate a value using the scoring method's validation
 * Checks Zod schema first (if provided), then validateValue function (if provided)
 * Returns true if valid, false otherwise
 */
export function validateScoringValue(
  scoringMethod: ScoringMethod,
  value: Value
): boolean {
  // First check Zod schema if provided
  if (scoringMethod.valueSchema) {
    try {
      scoringMethod.valueSchema.parse(value);
    } catch (error) {
  ...
  // Then check validateValue function if provided
  if (scoringMethod.validateValue) {
    return scoringMethod.validateValue(value);
  }
