...
/**
 * Team with account join
 */
export type TeamJoinWithAccount = TeamJoinBase & {
 ...
 */
export type TeamJoinFull = TeamJoinWithMembers & TeamJoinWithAccount;
...
/**
 * Team with all related data and nested sub-teams
...
export type TeamWithDetails = {
  ...
  // Account info (if this team has an account)
...
function buildTeamTree(
  ...
  for (const teamData of teams) {
    ...
    if (teamMap.has(teamId)) {
      // Update existing entry (merge account if present)
      const existing = teamMap.get(teamId)!;
      if (teamData.account) {
        existing.account = teamData.account;
      }
      continue;
    }
...
async function fetchAllSubTeams(
  ...
  teamId: string,
  depth: number = 0
): Promise<{ team: Participant; account?: Account | null; teamMembers: { teamMember: TeamMember; member: Participant }[] }[]> {
  if (depth > 10) {
  ...
  const accountResult = await db.select()
    ...
    .where(eq(schema.accounts.participantId, teamId))
    .limit(1);
  
  const account = accountResult.length > 0 ? accountResult[0] : null;
  
  const result: { team: Participant; account?: Account | null; teamMembers: { teamMember: TeamMember; member: Participant }[] }[] = [{
    team,
    account: account || null,
    teamMembers,
  }];
...
export function queryTeams(db: Database): TeamBuilder<{
  ...
  const createBuilder = <F extends JoinFlags>(): TeamBuilder<F> => ({
    ...
    withAccount() {
      if (!flags.account && selectQuery) {
        selectQuery = selectQuery
          .leftJoin(schema.accounts, eq(schema.participants.id, schema.accounts.participantId));
        flags.account = true;
      }
      return createBuilder<F & { account: true }>() as any;
    },
...
export async function upsertTeamWithDetails(
  ...
  await upsertEntity(db, schema.participants, {
    ...data.team,
    isTeam: true,
  } as Partial<ParticipantInsert>);
  
  // 2. Upsert account if provided
  if (data.account) {
    await upsertEntity(db, schema.accounts, {
      ...data.account,
      participantId: data.team.id,
    } as Partial<AccountInsert>);
  }
  ...
  if (allMemberIds.length > 0) {
    ...
    await upsertEntities(db, schema.teamMembers, teamMembers as Partial<TeamMemberInsert>[]);
