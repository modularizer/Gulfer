...
import type { VenueEventFormat, Venue, EventFormat, VenueEventFormatStage, VenueEventFormatStageInsert, EventFormatStage } from '../tables';
import { upsertEntity, upsertEntities, deleteMissingChildren } from '../query-builders';
import { queryStages, type StageWithDetails } from '../query-builders/0-stages';
...
export class VenueEventFormatService extends BaseService {
  ...
  private async createVenueStagesFromEventStages(
    ...
    for (const eventStage of eventFormatStages) {
      ...
      await upsertEntity(this.db, schema.venueEventFormatStages, venueEventFormatStage);
      ...
      for (const eventStage of eventStages) {
        ...
        if (metadata !== undefined) {
          ...
          if (existing) {
            ...
            await upsertEntity(this.db, schema.venueEventFormatStages, {
              ...existing,
              metadata: mergedMetadata,
