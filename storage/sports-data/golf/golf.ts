...
import { z } from 'zod';
import { Sport, type SportMetadataSchemas } from '../schema/sports';
import {
  sportMetadataSchema,
...
import type { StageInput, EventFormatWithDetails } from '../schema/services';
import type { VenueEventFormatWithDetails } from '../schema/services';
import { golfStrokePlay } from './scoring/stroke-play';
import { golfMatchPlay } from './scoring/match-play';
...
class Golf extends Sport {
  ...
  metadataSchemas: SportMetadataSchemas = {
    sport: sportMetadataSchema, // Use shared sport metadata schema
  ...
  async addCourse(
    ...
    let venue = (await this.venueService.getVenuesByName(venueName)).find(
      v => v.venue.name?.toLowerCase() === venueName.toLowerCase()
    );
    if (!venue) {
    ...
    const existingFormat = (await this.eventFormatService.getEventFormatsBySport(sportId)).find(
      ef => ef.name?.toLowerCase() === formatName.toLowerCase()
    );
    if (!existingFormat) {
    ...
    let venueEventFormat = venueEventFormats.find(
      vef => vef.eventFormatId === eventFormat.id
    );
