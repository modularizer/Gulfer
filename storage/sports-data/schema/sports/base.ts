...
import type { EventFormatWithDetails, StageInput } from '../services/event-format-service';
import type { VenueEventFormatWithDetails, StageMetadataInput } from '../services/venue-event-format-service';
...
import { VenueEventFormatService } from '../services/venue-event-format-service';
...
/**
 * Shared schema for sport metadata
 * All sports use this base schema
 */
...
export abstract class Sport {
  ...
  /**
   * Initialize the sport with database and services
  ...
  private async _registerScoringMethods(): Promise<void> {
    ...
    // Initialize and register each scoring method
    for (const scoringMethod of scoringMethodsSet) {
      await scoringMethod.initialize(this.db, sportId);
    }
  }
  ...
  private async _registerEventFormats(): Promise<void> {
    ...
    for (const formatDef of this.eventFormats) {
      ...
      if (existingFormat) {
        ...
        // Validate metadata if provided and schema exists
   ...
   * Handles generic venue creation with metadata validation
  ...
  async createVenue(
    ...
    const venueEventFormat = await this.venueEventFormatService.createVenueEventFormatWithStages(
      {
   ...
   * Handles generic event creation with metadata validation
   * Sports can override this method for custom behavior
   */
  async createEvent(
    event: Partial<Event>,
    ...
    // Default generic implementation with metadata validation
...
/**
 * Extract custom methods from a sport that aren't part of the base class
 * This is used for type inference when registering sports
 */
export type SportCustomMethods<T extends Sport> = {
  [K in keyof T as K extends keyof Sport
    ? never
