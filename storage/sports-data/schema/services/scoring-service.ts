...
import type { ParticipantEventStageScore, ParticipantEventStageScoreInsert, EventStage, ScoreFormat } from '../tables';
import { upsertEntity, upsertEntities } from '../query-builders';
import { queryStages } from '../query-builders/0-stages';
import { queryEvents, type EventWithDetails } from '../query-builders/1-events';
import { getScoringMethod } from '../scoring/registry';
import type { StageScoringInfo, EventScoringInfo, GroupResult } from '../scoring/base';
import { computeGroupResult, aggregateStageResults, validateScoringValue, defaultScoreStage, precomputeStageStats, recomputeStatsFromResult } from '../scoring/base';
...
export class ScoringService extends BaseService {
  ...
  async setStageScore(
    ...
    if (!validateScoringValue(scoringMethod, value)) {
    ...
    } as Partial<ParticipantEventStageScoreInsert>);
  ...
  private async recomputeStageScores(
    ...
    for (const update of scoreUpdates) {
      await upsertEntity(this.db, schema.participantEventStageScores, update);
    }
