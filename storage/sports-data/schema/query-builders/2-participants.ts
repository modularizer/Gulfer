...
import * as accountSchema from '../../../accounts/schema/tables';
...
import type { Account, AccountInsert } from '../../../accounts';
...
/**
 * Participant with account join
 */
export type ParticipantJoinWithAccount = ParticipantJoinBase & {
 ...
 */
export type ParticipantJoinFull = ParticipantJoinWithEvents & ParticipantJoinWithAccount;
...
export function queryParticipants(db: Database) {
  ...
  const builder = {
    ...
    withAccount() {
      if (!hasAccount) {
        selectQuery = selectQuery
          .leftJoin(accountSchema.accounts, eq(schema.participants.id, accountSchema.accounts.participantId));
        hasAccount = true;
      }
      return builder;
    },
...
export async function upsertParticipantWithDetails(
  ...
  await upsertEntity(db, schema.participants, data.participant as Partial<ParticipantInsert>);
  
  // 2. Upsert account if provided
  if (data.account) {
    await upsertEntity(db, accountSchema.accounts, {
      ...data.account,
      participantId: data.participant.id,
    } as Partial<AccountInsert>);
  }
  
  // 3. Upsert events and scores
  for (const eventData of data.events) {
    // Upsert the event
    await upsertEntity(db, schema.events, eventData.event as Partial<EventInsert>);
