import {DbConnectionInfo, DrizzleDatabaseConnectionDriver} from "./types";
...
export async function getConnectFunction<T extends DbConnectionInfo>(connInfo: T) {
    ...
    switch (driver) {
        case XPDriverName.PGLITE: {
            const { default: connectToPglite } = await import('./implementations/pglite');
            return connectToPglite as unknown as (info: T) => Promise<DrizzleDatabaseConnectionDriver<T>>;
        }
        case XPDriverName.POSTGRES: {
            const { default: connectToPostgres } = await import('./implementations/postgres');
            return connectToPostgres as unknown as (info: T) => Promise<DrizzleDatabaseConnectionDriver<T>>;
        }
        case XPDriverName.SQLITE_MOBILE: {
            const { default: connectToSqliteMobile } = await import('./implementations/sqlite-mobile');
            return connectToSqliteMobile as unknown as (info: T) => Promise<DrizzleDatabaseConnectionDriver<T>>;
        }
        default:
