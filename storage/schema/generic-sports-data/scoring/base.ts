...
export abstract class ScoringMethod {
    ...
    /**
     * Initialize the scoring method (no database registration)
     * This can be called before the database exists
     * Call register(db, sportId, scoreFormatService) later to register with the database
     */
    initialize(): void {
        // Ensure we're registered in the static registry
        this._ensureRegistered();
        // Services will be initialized when register() is called
        // Scoring methods can be instantiated without a database
    }

    /**
     * Register the scoring method with a database
     * Automatically registers itself in the database (upserts ScoreFormat)
     * 
     * @param db - Database instance
     * @param sportId - Optional sport ID to associate with
     * @param scoreFormatService - Optional score format service (will create if not provided)
     */
    async register(
        db: Database,
        sportId?: string,
        ...
        this.db = db;
        this.scoreFormatService = scoreFormatService || new ScoreFormatService(db);
    ...
    private async _selfRegister(sportId?: string): Promise<void> {
        ...
        if (existing.length > 0 && sportId) {
            ...
            // If no sportId specified, use the first one found (or create a generic-sports one)
    ...
    /**
     * Zod generic-sports for validating score values
    ...
    validateValue(
        ...
        // First check Zod generic-sports if provided
        if (this.valueSchema) {
            try {
