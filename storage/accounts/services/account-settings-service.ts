...
import { BaseService } from '../../sports-data/schema/services/base';
...
import { upsertEntity } from '../../sports-data/schema/query-builders';
...
export class AccountSettingsService extends BaseService {
  ...
  async setAccountSetting(
    ...
    // Note: Full Zod validation would require storing the schema in a way that can be reconstructed
  ...
  async getAccountSetting(
    ...
    if (results.length === 0) {
      // Return null if not set (defaults would be handled by the Zod schema)
      return null;
    }
   ...
   * Note: This requires the schema to be stored in a way that can be reconstructed
