 ...
 * 
 * @deprecated Use db.deleteMissingChildren() or table.deleteMissingChildren() instead
 * 
...
export async function deleteMissingChildren(
  ...
  // Delegate to the method on the database or table instance
  if (table && typeof table.deleteMissingChildren === 'function') {
    return table.deleteMissingChildren(parentIdColumn, parentId, keepIds);
  } else if (db && typeof db.deleteMissingChildren === 'function') {
    return db.deleteMissingChildren(table, parentIdColumn, parentId, keepIds);
  } else {
    throw new Error('deleteMissingChildren is not available. Use XPDatabaseConnectionPlus or XPDatabaseTablePlus instance.');
  }
}
