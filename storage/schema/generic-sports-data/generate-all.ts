 ...
 * 2. SQL migrations (migrations/*.sql) - includes both generic-sports-data and accounts tables
...
/**
 * Generate all generic-sports artifacts
 */
function generateAll(): void {
  console.log('🚀 Generating all generic-sports artifacts...\n');
  ...
  try {
    ...
    console.log('🎉 All generic-sports artifacts generated successfully!');
    console.log('   - TypeScript types updated');
    console.log('   - SQL migrations created');
    console.log('   - Migrations index updated');
    console.log('\n   Ready to commit!');
  } catch (error) {
    console.error('❌ Error generating generic-sports artifacts:', error);
    process.exit(1);
  }
}
